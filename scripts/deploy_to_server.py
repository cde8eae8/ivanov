...
def run_remote_deploy(python_exe, target_dir, ssh):
    ...
    unpack_command = [python_exe, target_dir / 'deploy_to_server.py', '--mode=unpack', '--target-dir', target_dir]
...
def local_main(args):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if args.target_machine != 'localhost':
            ...
            ssh.exec_command(shlex.join(['mkdir', str(pathlib.PurePosixPath(args.target_dir))]))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-C", default=pathlib.Path(__file__).parent.parent, dest='working_dir')
