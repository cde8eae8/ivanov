...
def test_send_phrases(tmp_path, testing_db):
    with testing_db.session() as session:
        ...
        session.execute(sqlalchemy.insert(models.UsedPhrases).values({"user_id": user.id, "phrase_id": phrase.id}))
        session.commit()
        for u in users:
            session.refresh(u)
    ...
    class Observer(test.bot.MockTelebotObserver):
        def on_message(self, sent_by_bot, message: test.bot.Message):
            ...
            if (len(test_bot.chats[user.chat_id]) >= 3 and
                len(test_bot.chats[user2.chat_id]) >= 3 and
                len(test_bot.chats[admin.chat_id]) >= 1):
               loop.stop()
